...
use crate::{
    commands::shared::{
        bundle_entries, concept_text, first_string, iso_on_or_after, open_patient_session, resolve_since_floor,
    },
    Result,
};
...
fn run_audit(args: ClaimsAuditArgs, context: &crate::state::ResolvedContext) -> Result<Value> {
    ...
    for resource in claims {
        ...
        let provider = first_string(&resource, &["/provider/display"]).unwrap_or_else(|| "Unknown provider".into());
