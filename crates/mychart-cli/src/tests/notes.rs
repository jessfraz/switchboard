        ...
        ResponseSpec::json(
            ...
            json!({
                ...
                "data": crate::base64_encode(
                    br#"<ClinicalDocument><recordTarget><patientRole><patient><name><given>Casey</given><given>Alex</given><family>Morgan</family></name></patient></patientRole></recordTarget><section><title>Progress Note</title><text>Patient reports migraine improvement after medication change.</text></section></ClinicalDocument>"#
                )
            }),
            Vec::new(),
        ),
...
fn notes_get_normalizes_xml_boundary_mush() {
    let server = TestServer::spawn(vec![
        ...
        ResponseSpec::json(
            ...
            json!({
                ...
                "contentType": "application/xml",
                "data": crate::base64_encode(
                    br#"<ClinicalDocument><recordTarget><patientRole><addr><streetAddressLine>123 Example Street</streetAddressLine><city>Example City</city><state>ZZ</state></addr><patient><name><given>Casey</given><given>Alex</given><family>Morgan</family></name></patient></patientRole></recordTarget><section><title>After Visit Summary</title><text>Submitted on (January 2, 2100)Example follow-up text.</text></section></ClinicalDocument>"#
                )
            }),
            Vec::new(),
        ),
    ...
    assert!(
        body.contains("Submitted on (January 2, 2100) Example follow-up text."),
        "body was: {body}"
    );
    assert!(!body.contains("Casey Alex Morgan"));
    assert!(!body.contains("123 Example Street"));
    assert!(!body.contains("2100)Example"));
}
...
<ClinicalDocument xmlns="urn:hl7-org:v3">
  ...
  <recordTarget>
    <patientRole>
      <addr>
        <streetAddressLine>123 Example Street</streetAddressLine>
          ...
          <text>
            <table>
              <tbody>
                ...
                <tr><td>Example measurement</td><td>3.0</td></tr>
              </tbody>
            </table>
          </text>
        </section>
...
"#;
    ...
    assert!(body.contains("Results\nTest Value\nExample measurement 3.0"));
    assert!(!body.contains("123 Example Street"));
    assert!(!body.contains("Casey Morgan"));
}
